"""
This is basically the same script as bp_full_polynomial, but it takes data from a csv file and fits a polynomial to it.
Then, in <EXAMINE> the input data from the csv is noised up, and run through the fitted polynomial to generate another 
    csv that hopefully replicates behavior seen in the original data.
    
This is built to take csv generated by the spider,
     so it takes the first two columns (data) and the second to the last column (time) as the independent data.
     and the final column as the sensor/dependent data.

The first argument passed to the script is the source csv, the second is the destination csv for polynomial generated data.
"""
...
#test_sansBetas = pre_beta(indData, subsetIndices, n) #use original data.

#generate dependent data from the fitted poly.
fromFitted = np.einsum('ij,...ij', betas.random(), test_sansBetas)

#gather arrays to send to the csv, transpose back for better csv display.
...
np.savetxt(destCSV, toCSV, fmt='%.3f', delimiter=",")
